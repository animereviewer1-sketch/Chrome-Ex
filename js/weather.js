 ...
 * Default location: München, Haidhausen
 */
const Weather = {
  widgetElement: null,
  tempElement: null,
  ...
  async loadSettings() {
    ...
    // Default to München, Haidhausen if no location is set
    this.location = await Storage.get('weatherLocation', 'München, Haidhausen');
    this.tempUnit = await Storage.get('tempUnit', 'c');
    
    // Save default location if not set
    const savedLocation = await Storage.get('weatherLocation', null);
    if (!savedLocation) {
    ...
    if (this.apiKey && this.location) {
      ...
      // Show default location without weather data
      this.locationElement.textContent = 'München, Haidhausen';
    }
  },
